...
public abstract class BaseFragment<M extends  BaseModel,P extends BasePresenter> extends RxFragment {
    ...
    private boolean isViewPrepared; // 标识fragment视图已中的view经初始化完毕
...
//    初始化事件Event
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (rootView != null) {
            ...
            if (parent != null) {
                parent.removeView(rootView);//删除已经加载过的并且存在的视图,然后把视图冲洗加载上去,避免
                //加载过多相同的视图,曹成内存消耗.
            }
            return rootView;
        }
    ...
    private <T extends BasePresenter> T getPresenterImpl()
    {
