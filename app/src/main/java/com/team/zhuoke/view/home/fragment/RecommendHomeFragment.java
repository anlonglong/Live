...
public class RecommendHomeFragment extends BaseFragment<HomeRecommendModelLogic, HomeRecommendPresenterImp> implements HomeRecommendContract.View, BGABanner.Delegate<SimpleDraweeView, String> {
    ...
    protected void setXrefeshViewConfig() {
        rtefreshContent.setPinnedTime(2000);//刷新以后停留的时间
...
//        recommed_banner.setDelegate(this);
        ArrayList<String> pic_url = getBannerList(mHomeCarousel);
        if (recommed_banner != null && null !=pic_url &&pic_url.size() > 0) {
    ...
    private ArrayList<String> getBannerList(List<HomeCarousel> mHomeCarousel) {
        ArrayList<String> pic_urls = null;
        if (null != mHomeCarousel && !mHomeCarousel.isEmpty()) {
            pic_urls = new ArrayList<>();
            for (int i = 0; i < mHomeCarousel.size(); i++) {
                pic_urls.add(mHomeCarousel.get(i).getPic_url());
                LogUtil.i("item_banner_url = ",mHomeCarousel.get(i).getPic_url());
            }
        }
        return pic_urls;
    }
    ...
    public void getViewHotColumn(List<HomeHotColumn> mHomeHotColumn) {
        adapter.setHomeHotColumn(mHomeHotColumn);
    }

    //颜值
    @Override
    public void getViewFaceScoreColumn(List<HomeFaceScoreColumn> homeFaceScoreColumns) {
        adapter.setFaceScoreColmun(homeFaceScoreColumns);
    }
...
//        去掉颜值栏目
        ...
        adapter.setAllColumn(homeRecommendHotCates);
    }
